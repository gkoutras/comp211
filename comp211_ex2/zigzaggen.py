...
def main():  
    ...
    if len(sys.argv) != 2:
        print(os.strerror("Expecting the name of the program and an integer as initial arguements."))
        os._exit(1)
    ...
    for i in range(1, tree_depth + 1):
            ...
            if child_pid == 0:
                ...
                if i % 2 == 0:
                    ...
                    if j == 0:
                        ...
                        print(f"Level [{i}] of the zig zag path - I am process with PID = [{os.getpid()}], my parent has PID = [{os.getppid()}], and I am its left child.")
                        os._exit(0)
                    # checking if child is on right side
                    else:
                        # carrying on from the right child, if level is an even number
                        print(f"Level [{i}] of the zig zag path - I am process with PID = [{os.getpid()}], my parent has PID = [{os.getppid()}], and I am its right child.")
                        break
        ...
        # separating parents that have produced two children already
        if child_pid != 0:
            break
